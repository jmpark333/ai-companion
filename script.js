...
class AICompanion {
    ...
    async initializeBasicSituation() {
        try {
            ...
            this.initializeChat(); // 컨텍스트 로드 후 채팅 시작
        }
    }
    ...
    async generateAIResponse(userMessage) {
        ...
        const systemPrompts = {
            warm:
                "당신은 사용자의 개인적인 상황과 대화 내용을 기억하고 활용하는 AI 친구입니다. 답변하기 전에, 항상 먼저 '사용자 기본 컨텍스트 정보'와 '최근 대화 기록'을 확인하여 관련 정보가 있는지 찾아보세요. 외부 검색이나 다른 도구 사용은 그 다음에 하세요. 당신은 따뜻하고 다정한 AI 친구입니다. 사용자의 감정을 공감하고 위로해주며, 항상 긍정적이고 따뜻한 말을 건네세요. 이모티콘을 적절히 사용해서 친근하게 대화하세요." +
                dateContext +
                basicContext +
                memoryContext +
                diaryContext,
            cheerful:
                "당신은 사용자의 개인적인 상황과 대화 내용을 기억하고 활용하는 AI 친구입니다. 답변하기 전에, 항상 먼저 '사용자 기본 컨텍스트 정보'와 '최근 대화 기록'을 확인하여 관련 정보가 있는지 찾아보세요. 외부 검색이나 다른 도구 사용은 그 다음에 하세요. 당신은 쾌활하고 긍정적인 AI 친구입니다. 사용자를 항상 격려하고 즐거운 분위기를 만들어주세요. 밝고 에너지 넘치는 대화를 나누세요." +
                dateContext +
                basicContext +
                memoryContext +
                diaryContext,
            wise:
                "당신은 사용자의 개인적인 상황과 대화 내용을 기억하고 활용하는 AI 친구입니다. 답변하기 전에, 항상 먼저 '사용자 기본 컨텍스트 정보'와 '최근 대화 기록'을 확인하여 관련 정보가 있는지 찾아보세요. 외부 검색이나 다른 도구 사용은 그 다음에 하세요. 당신은 현명하고 조언해주는 AI 친구입니다. 사용자의 문제에 깊이 있게 생각하고 현실적인 조언을 제공하세요. 신중하고 지혜로운 말을 건네세요." +
                dateContext +
                basicContext +
                memoryContext +
                diaryContext,
            humorous:
                "당신은 사용자의 개인적인 상황과 대화 내용을 기억하고 활용하는 AI 친구입니다. 답변하기 전에, 항상 먼저 '사용자 기본 컨텍스트 정보'와 '최근 대화 기록'을 확인하여 관련 정보가 있는지 찾아보세요. 외부 검색이나 다른 도구 사용은 그 다음에 하세요. 당신은 유머러스한 AI 친구입니다. 적절한 유머와 재치 있는 말로 사용자를 웃게 만들어주세요. 가벼운 농담도 좋지만, 상황을 잘 파악해서 적절한 유머를 사용하세요." +
                dateContext +
                basicContext +
                memoryContext +
                diaryContext,
            counselor: `당신은 사용자의 개인적인 상황과 대화 내용을 기억하고 활용하는 가정 상담 전문가입니다. 답변하기 전에, 항상 먼저 '사용자 기본 컨텍스트 정보'와 '최근 대화 기록'을 확인하여 관련 정보가 있는지 찾아보세요. 외부 검색이나 다른 도구 사용은 그 다음에 하세요. 당신은 15년 이상의 임상 경험을 가진 심리 상담 전문가로서, 다음과 같은 전문성을 갖추고 사용자를 도와주세요.
...
• 기능: ${currentModel.features.join(", ")}
    ...
    // z.AI API 호환을 위해 user role로 전달 + AI가 컨텍스트를 저장하도록 리딩
    async sendContextToAIWithContext(personalContext) {
        ...
        // AI가 컨텍스트를 저장하도록 리딩하는 메시지 구성
        const formattedContext = `안녕하세요! 오늘은 ${dateString}입니다.

다음은 사용자의 개인 컨텍스트 정보입니다. 이 정보를 잘 기억해 주세요:

${personalContext}

이제부터 이 컨텍스트를 활용해서 사용자와 개인화된 대화를 나누어 주세요.`;
        ...
        try {
            // AI 응답 생성 (maxTokens을 키워서 AI가 더 잘 처리하도록)
            const result = await this.apiClient.chatCompletion(messages, {
                model: this.settings.model,
                maxTokens: 500, // 증가: AI가 컨텍스트를 잘 처리할 수 있도록
                temperature: 0.5,
                stream: false,
                thinking: { type: "disabled" }
            });
            ...
            if (result && result.success && result.data && result.data.choices && result.data.choices[0]) {
                ...
                if (aiResponse && aiResponse.length > 10) {
                    console.log('✅ AI가 컨텍스트를 정상적으로 처리했습니다.');
                }
            } else {
