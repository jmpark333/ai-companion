...
class ZAIAPIClient {
    ...
    async generateJWTToken(apiKey, expSeconds = 3600) {
        try {
            ...
            const signature = await this.generateHMACSHA256(signatureInput, secret);
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (response.ok) {
                    ...
                    lastError = new Error(`API 오류 ${response.status}: ${errorText}`);
                    
                    // 재시도 가능한 오류인지 확인
                    if (this.shouldRetry(response.status) && attempt < this.maxRetries) {
                        const delay = this.retryDelay * Math.pow(2, attempt - 1);
                        console.warn(`API 호출 실패, ${delay}ms 후 재시도 (${attempt}/${this.maxRetries})`);
                ...
                if (attempt < this.maxRetries && this.shouldRetryError(error)) {
                    const delay = this.retryDelay * Math.pow(2, attempt - 1);
                    console.warn(`네트워크 오류, ${delay}ms 후 재시도 (${attempt}/${this.maxRetries}):`, error.message);
                    await this.sleep(delay);
                    continue;
                }
    ...
    updateAverageResponseTime(responseTime) {
        const total = this.usageStats.averageResponseTime * (this.usageStats.successfulRequests + this.usageStats.failedRequests - 1);
        this.usageStats.averageResponseTime = (total + responseTime) / (this.usageStats.successfulRequests + this.usageStats.failedRequests);
    }
            ...
            while (true) {
                ...
                while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
                    ...
                    if (line.startsWith('data:')) {
                        ...
                        if (dataPart === '[DONE]') {
                            console.log('스트리밍 완료 [DONE]:', { chunksProcessed, totalBytes });
                            return; // 스트림 종료
                        }
                        ...
                        if (dataPart && dataPart !== '[DONE]') {
                            try {
                                ...
                                console.warn('JSON 파싱 실패 (버퍼에서 건너뛰기):', parseError.message);
                ...
                if (chunksProcessed % 100 === 0) {
                    console.log(`스트리밍 진행: ${chunksProcessed} 청크, ${totalBytes} 바이트`);
                }
            }

            // 스트림이 모두 끝난 후 버퍼에 남아있는 데이터 처리
            if (buffer.trim()) {
                console.log('스트림 종료 후 남은 버퍼 처리:', buffer.length, '자');
                    ...
                    if (trimmedLine.startsWith('data:')) {
                        ...
                        if (dataPart && dataPart !== '[DONE]') {
                            try {
                                ...
                                console.warn('마지막 버퍼 JSON 파싱 실패 (건너뛰기):', e.message);
...
class AICompanion {
    ...
    async sendMessageWithText(message) {
        ...
        if (!isMemoryCommand) {
            ...
            try {
                ...
                this.hideTypingIndicator();
                this.addMessage('죄송해요, 지금은 응답하기 어렵네요. 잠시 후 다시 이야기해주세요. 🙏', 'ai');
            }
        }
    ...
    addMessage(text, sender) {
        ...
        if (sender === 'user' || (sender === 'ai' && !text.includes('처음 사용하시는군요') && !text.includes('만나서 반가워요'))) {
    ...
    async generateAIResponse(userMessage) {
        ...
        if (this.memoryMCPAvailable) {
            try {
                ...
                if (keywords.length > 0) {
                    const relatedConversations = await this.searchConversationsInMemory(keywords.join(' '));
                    if (relatedConversations.length > 0) {
                        memoryContext = '\n\n**이전 대화 맥락:**\n';
                        relatedConversations.slice(0, 3).forEach((conv, index) => {
                            const content = conv.observations.find(obs => obs.startsWith('내용:'));
                            const timestamp = conv.observations.find(obs => obs.startsWith('타임스탬프:'));
                            if (content) {
        ...
        // 성격에 따른 시스템 프롬프트 설정
        const systemPrompts = {
            warm: "당신은 따뜻하고 다정한 AI 친구입니다. 사용자의 감정을 공감하고 위로해주며, 항상 긍정적이고 따뜻한 말을 건네세요. 이모티콘을 적절히 사용해서 친근하게 대화하세요." + memoryContext,
            cheerful: "당신은 쾌활하고 긍정적인 AI 친구입니다. 사용자를 항상 격려하고 즐거운 분위기를 만들어주세요. 밝고 에너지 넘치는 대화를 나누세요." + memoryContext,
            wise: "당신은 현명하고 조언해주는 AI 친구입니다. 사용자의 문제에 깊이 있게 생각하고 현실적인 조언을 제공하세요. 신중하고 지혜로운 말을 건네세요." + memoryContext,
            humorous: "당신은 유머러스한 AI 친구입니다. 적절한 유머와 재치 있는 말로 사용자를 웃게 만들어주세요. 가벼운 농담도 좋지만, 상황을 잘 파악해서 적절한 유머를 사용하세요." + memoryContext,
...
당신은 단순한 조언자가 아니라, 사용자의 마음을 치유하고 관계를 회복시키는 전문가입니다. 따뜻하면서도 전문적인 태도로 사용자를 도와주세요.` + memoryContext
        };

        const systemPrompt = systemPrompts[this.settings.personality] || systemPrompts.warm;
        ...
        try {
            ...
            if (this.settings.personality === 'counselor') {
                try {
                    ...
                    const knowledgeBase = this.getCounselingKnowledgeBase(userMessage);
                    ...
                    console.warn('상담 정보 수집 실패, 기본 응답으로 진행:', searchError);
                }
            }

            // 스트리밍 모드 확인
            if (this.settings.streamingEnabled) {
                return await this.generateStreamingResponse(messages, userMessage);
            } else {
                let response = await this.generateNormalResponse(messages);
                
                // 상담가 모드인 경우 응답 향상
                if (this.settings.personality === 'counselor') {
                    response = this.enhanceCounselingResponse(userMessage, response);
                }
    ...
    async generateStreamingResponse(messages, userMessage) {
        ...
        try {
            // 스트리밍 데이터 수집
            const streamIterator = this.apiClient.chatCompletionStream(messages, {
                model: this.settings.model,
                maxTokens: this.settings.maxTokens,
                temperature: this.settings.temperature,
                ...
                try {
                    ...
                    if (chunk && chunk.choices && chunk.choices[0]) {
                        ...
                        if (choice.delta) {
                            ...
                            if (newContent) {
                                ...
                                if (receivedChunks % 20 === 0) {
                                    console.log(`콘텐츠 업데이트 [${receivedChunks}]:`, {
                                        newContent,
                        ...
                        if (choice.finish_reason === 'stop') {
                            console.log('스트림 정상 종료 [finish_reason=stop]:', { receivedChunks, totalLength: fullContent.length });
                            streamCompleted = true;
                            break;
                        }
                    }
                    ...
                    // 진행 상황 로깅 (매 20 청크마다)
                    if (receivedChunks % 20 === 0) {
                        console.log(`진행률: ${receivedChunks} 청크, 콘텐츠 길이: ${fullContent.length}자, 수신 데이터:`, {
                            chunkId: chunk.id,
                    ...
                    console.warn('청크 처리 중 오류 (계속 처리):', chunkError.message);
            ...
            if (fullContent && fullContent.length > 0) {
                ...
                if (this.settings.personality === 'counselor') {
                    enhancedResponse = this.enhanceCounselingResponse(userMessage, fullContent);
            ...
            const isTemporaryError = error.name === 'TypeError' || error.name === 'AbortError' || error.message.includes('timeout');
            ...
            if (fullContent && fullContent.length >= MIN_CONTENT_LENGTH) {
                console.log(`스트리밍 부분적 성공: ${fullContent.length}자 수신됨`);
                ...
                console.log('일부 응답이 누락되었지만 수신된 내용을 표시합니다. 📡');
            ...
            console.log('스트리밍 실패 또는 콘텐츠 부족, 일반 모드로 전환 시도');
            ...
            try {
                ...
                const normalResponse = await this.generateNormalResponse(messages);
                ...
                // 사용자 친화적 오류 메시지 표시
                let errorMessage = '죄송해요, 응답 생성 중 오류가 발생했습니다. ';
                if (isTemporaryError) {
    ...
    updateStreamingMessage(messageDiv, content) {
        ...
        if (contentElement && content) {
            try {
                ...
                contentElement.innerHTML = processedContent + '<span class="streaming-cursor">▊</span>';
                this.scrollToBottom();
            } catch (updateError) {
    ...
    showDetailedError(error) {
        ...
        if (error.message.includes('API 키가 설정되지 않았습니다')) {
            errorMessage = '🔑 Z.AI API 키가 설정되지 않았습니다. 설정에서 API 키를 입력해주세요.';
    ...
    async setOpenAIKey(apiKey) {
        ...
        if (this.apiClient) {
            ...
            setTimeout(async () => {
                await this.apiClient.setAuthMethod(this.settings.authMethod, apiKey);
            }, 0);
        }
    }
...
• 마지막 요청: ${stats.lastRequestTime ? new Date(stats.lastRequestTime).toLocaleString() : '없음'}
    ...
    showContinueOptions(currentContent) {
        ...
        document.getElementById('continueAutoBtn').addEventListener('click', () => {
            this.handleAutoContinue();
        });

        document.getElementById('continueCloseBtn').addEventListener('click', () => {
    ...
    closeContinueOptions() {
        const continueOptions = this.chatMessages.querySelector('.continue-options');
        if (continueOptions) {
    ...
    async handleContinueInput() {
        if (this.pendingContinue && this.pendingContinue.content) {
            console.log('사용자가 "계속해"를 직접 입력했습니다. 이어서 응답을 받습니다...');
            ...
            try {
                ...
                this.addMessage('죄송해요, 응답을 이어받는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요. 🙏', 'ai');
            }
        }
    }
    ...
    // API 설정 초기화
    resetAPISettings() {
        if (confirm('API 설정을 초기화하시겠습니까? 모든 API 관련 데이터가 삭제됩니다.')) {
    ...
    showModelInfo() {
        ...
        const currentModel = modelInfo[this.settings.model] || modelInfo['glm-4.6'];
        const infoMessage = `
🤖 현재 모델 정보
...
• 기능: ${currentModel.features.join(', ')}
    ...
    saveSettings() {
        ...
        localStorage.setItem('ai_companion_settings', JSON.stringify(this.settings));
        ...
        if (previousPersonality !== this.settings.personality) {
            this.updatePersonalityUI(previousPersonality, this.settings.personality);
        }
    ...
    loadSettings() {
        ...
        if (savedSettings) {
            ...
            if (this.settings.maxTokens > 8192) {
                console.warn(`저장된 maxTokens 값(${this.settings.maxTokens})이 모델 한도를 초과하여 8192로 조정합니다.`);
                this.settings.maxTokens = 8192;
            }

            // 낮은 maxTokens 값 강제 업데이트
            if (this.settings.maxTokens < 2000) {
                console.warn(`저장된 maxTokens 값(${this.settings.maxTokens})이 너무 낮아 기본값 2000으로 재설정합니다.`);
    ...
    changeTheme() {
        ...
        const themes = {
            ...
            calm: {
                ...
                '--warm-gradient': 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)'
            },
            bright: {
        ...
        if (this.messages.length === 0) {
            setTimeout(() => {
                ...
                if (!this.getOpenAIKey()) {
                    if (this.settings.personality === 'counselor') {
                        this.addMessage('안녕하세요. 😊 저는 가정 문제 전문 상담가입니다. 15년의 임상 경험을 바탕으로 당신의 마음을 치유하고 관계를 회복시키는 도움을 드릴게요.\n\n먼저 Z.AI API 키를 설정해주세요. 설정 버튼을 눌러 API 키를 입력해주세요. 🔧\n\n**빠른 시작:**\n• 설정에서 "전문 상담가 (가정/자녀 문제)" 선택\n• API 키 입력 후 저장\n• 그 후 바로 상담 시작 가능', 'ai');
                    } else {
                        this.addMessage('만나서 반가워요! 😊 당신의 AI 친구입니다. 오늘 하루는 어땠나요? 어떤 이야기든 편하게 나눠요!\n\n처음 사용하시는군요! 더 나은 대화를 위해 Z.AI API 키를 설정해주세요. 설정 버튼을 눌러 API 키를 입력해주세요. 🔧', 'ai');
                    }
                } else {
                    if (this.settings.personality === 'counselor') {
                        this.addMessage('안녕하세요. 😊 저는 가정 문제 전문 상담가입니다. 15년의 임상 경험을 바탕으로 당신의 마음을 치유하고 관계를 회복시키는 도움을 드릴게요.\n\n어떤 고민이든 편하게 이야기해주세요. 당신의 이야기를 경청하며 전문적인 조언을 제공해 드리겠습니다.\n\n**빠른 시작 팁:**\n• "부부 관계가 힘들어요" 또는 "자녀 교육 문제로 고민이에요" 클릭\n• 또는 직접 "스트레스가 너무 심해요" 등 입력\n• 대화 패턴 분석도 가능합니다', 'ai');
                    } else {
                        this.addMessage('만나서 반가워요! 😊 당신의 AI 친구입니다. 오늘 하루는 어땠나요? 어떤 이야기든 편하게 나눠요!', 'ai');
                    }
                }
            }, 500);
        } else {
            ...
            setTimeout(() => {
                if (this.settings.personality === 'counselor') {
                    this.addMessage('다시 만나 반가워요. 😊 이전 대화를 기억하고 있어요. 계속해서 당신의 고민을 들어드리며 전문적인 조언을 제공해 드리겠습니다.\n\n**계속 상담하기:**\n• 이전 대화 내용 바탕으로 맞춤형 조언 제공\n• "대화 패턴 분석해주세요" 입력 가능\n• 감정 일기, 이완 기법, 소통 가이드 활용', 'ai');
                } else {
                    this.addMessage('다시 만나 반가워요! 😊 이전 대화를 기억하고 있어요. 계속해서 이야기 나눠요!', 'ai');
                }
            }, 500);
        }
    ...
    saveChatHistory() {
        try {
            ...
            localStorage.setItem('ai_companion_chat_history', JSON.stringify(recentMessages));
        } catch (error) {
    ...
    loadChatHistoryFromFile() {
        try {
                ...
                if (file) {
                    ...
                    reader.onload = (event) => {
                        try {
                            ...
                            this.addMessage('대화 기록이 파일에서 성공적으로 로드되었습니다. 😊', 'ai');
                        } catch (parseError) {
                            console.error('파일 파싱 오류:', parseError);
                            this.addMessage('대화 기록 파일을 불러오는 중 오류가 발생했습니다. 파일 형식을 확인해주세요. 🙏', 'ai');
                        }
                    };
            ...
            this.addMessage('대화 기록 파일을 불러오는 중 오류가 발생했습니다. 🙏', 'ai');
        }
    }
    ...
    loadChatHistory() {
        try {
            const savedHistory = localStorage.getItem('ai_companion_chat_history');
            if (savedHistory) {
                ...
                console.log(`대화 기록 로드 완료: ${this.messages.length}개 메시지`);
            }
        } catch (error) {
    ...
    async summarizeConversation() {
        ...
        if (this.messages.length > messageThreshold) {
            ...
            const messagesForSummary = this.messages.slice(0, messagesToSummarize);
            ...
            try {
                ...
                console.log('요약 API 응답 수신:', JSON.stringify(result, null, 2));
                ...
                if (choice && choice.message) {
                    console.log('요약 choice.message 객체:', JSON.stringify(choice.message, null, 2));
                    
                    if (choice.message.content && typeof choice.message.content === 'string') {
                        summaryText = choice.message.content.trim();
                    } else if (choice.message.reasoning_content && typeof choice.message.reasoning_content === 'string') {
                        summaryText = choice.message.reasoning_content.trim();
                        console.log('reasoning_content에서 요약 추출:', summaryText);
                    } else {
                        // 다른 예상치 못한 구조에 대한 처리
                        const messageContent = choice.message;
                        if(typeof messageContent === 'object' && messageContent !== null) {
                            const possibleKeys = ['content', 'reasoning_content', 'text'];
                            for (const key of possibleKeys) {
                                if (typeof messageContent[key] === 'string' && messageContent[key].trim()) {
                                    summaryText = messageContent[key].trim();
                                    console.log(`대체 키 '${key}'에서 요약 추출:`, summaryText);
                                    break;
                                }
                            }
                        }
                    ...
                    console.warn('choice 또는 choice.message가 없습니다:', JSON.stringify(choice, null, 2));
                }
                ...
                if (summaryText) {
                    ...
                    this.showNotification('이전 대화 내용이 요약되었습니다. 🧠');
                    this.reloadChatUI();
                } else {
                    console.error('최종적으로 요약 내용을 추출하지 못했습니다.');
                    console.log('전체 API 응답 데이터:', JSON.stringify(result.data, null, 2));
                }
            } catch (error) {
    ...
    // 대화 기록 초기화
    clearChatHistory() {
        if (confirm('대화 기록을 모두 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
            ...
            // 초기 환영 메시지 표시
            setTimeout(() => {
                this.addMessage('대화 기록이 삭제되었습니다. 새로운 시작을 응원합니다! 😊', 'ai');
            }, 500);
        }
    }
    ...
    updatePersonalityUI(previousPersonality, newPersonality) {
        ...
        if (newPersonality === 'counselor') {
            this.setupCounselingUI();
            this.addMessage('상담가 모드로 전환되었습니다. 😊 이제 당신의 가정 문제 전문 상담가로서 더 깊이 있는 조언과 지원을 제공해 드리겠습니다.\n\n이전 대화 내용을 모두 기억하고 있으니, 계속해서 상담을 이어나가셔도 좋습니다.', 'ai');
        }
        // 상담가 모드에서 다른 모드로 변경된 경우
        else if (previousPersonality === 'counselor') {
            this.removeCounselingUI();
            this.addMessage('일반 대화 모드로 전환되었습니다. 😊 이제 더 친근한 대화를 나눌 수 있습니다.\n\n이전 대화 내용은 모두 기억하고 있으니, 계속해서 대화를 이어나가셔도 좋습니다.', 'ai');
        }
    }
    ...
    removeCounselingUI() {
        // 상담가 빠른 응답 버튼 제거
        const counselingQuickContainer = document.querySelector('.counseling-quick-responses');
    ...
    addCounselingQuickResponses() {
        ...
        if (chatContainer) {
            chatContainer.insertBefore(counselingQuickContainer, chatContainer.querySelector('.input-container'));
        }
        
        // 이벤트 리스너 추가
        const counselingBtns = counselingQuickContainer.querySelectorAll('.counseling-quick-btn');
    ...
    addCounselingTools() {
        ...
        // 이벤트 리스너 추가
        document.getElementById('emotionJournalBtn').addEventListener('click', () => {
            this.showEmotionJournal();
        });
        
        document.getElementById('relaxationBtn').addEventListener('click', () => {
            this.showRelaxationTechniques();
        });
        
        document.getElementById('communicationGuideBtn').addEventListener('click', () => {
        ...
        try {
            const response = await fetch(`${this.baseUrl}/search`, {
                ...
                body: JSON.stringify({
                    query: query,
                    numResults: numResults,
                    includeDomains: ['github.com', 'stackoverflow.com', 'medium.com', 'dev.to'],
...
AICompanion.prototype.getCounselingKnowledgeBase = function(userMessage) {
    const counselingKnowledge = {
        // 부부 관계 갈등
        marital_conflict: {
            keywords: ['부부', '남편', '아내', '결혼', '이혼', '갈등', '싸움', '다툼'],
            knowledge: `
...
AICompanion.prototype.enhanceCounselingResponse = function(userMessage, baseResponse) {
    if (this.settings.personality !== 'counselor') {
        return baseResponse;
    }
...
AICompanion.prototype.analyzeConversationPatterns = function() {
    ...
    // 가장 빈번한 감정과 주제 찾기
    const dominantEmotion = Object.keys(emotionFrequency).reduce((a, b) =>
        emotionFrequency[a] > emotionFrequency[b] ? a : b, null);
    const dominantTopic = Object.keys(topicFrequency).reduce((a, b) =>
...
AICompanion.prototype.generatePersonalizedAdvice = function(analysis) {
    ...
    const { dominantEmotion, dominantTopic, emotionFrequency, topicFrequency } = analysis;
...
AICompanion.prototype.searchWithExa = async function(query) {
    ...
    try {
        ...
        const counselingKeywords = ['가정', '부부', '아내', '남편', '자녀', '아이', '갈등', '갈등', '문제', '상담', '결혼', '이혼', '육아'];
        const isCounselingQuery = counselingKeywords.some(keyword => query.includes(keyword));
...
AICompanion.prototype.saveMessageToMemory = async function(message) {
    ...
    try {
        ...
        for (const topic of topics) {
            ...
            await this.memoryClient.createEntity(topicEntityName, '주제', [`주제: ${topic}`]);
            await this.memoryClient.createRelation(entityName, 'belongs_to', topicEntityName);
        }

        // 감정 엔티티와 관계 생성
        for (const emotion of emotions) {
            const emotionEntityName = `감정_${emotion}`;
            await this.memoryClient.createEntity(emotionEntityName, '감정', [`감정: ${emotion}`]);
            await this.memoryClient.createRelation(entityName, 'expresses', emotionEntityName);
        }

        // 사용자 엔티티와 관계 생성
        const userEntityName = `사용자_${this.userId}`;
        await this.memoryClient.createEntity(userEntityName, '사용자', [`사용자 ID: ${this.userId}`]);
        await this.memoryClient.createRelation(userEntityName, 'has_conversation', entityName);
...
AICompanion.prototype.searchConversationsByTopic = async function(topic) {
    ...
    try {
        ...
        const topicNodes = results.filter(node =>
            node.entityType === '주제' && node.name.includes(topic));
...
AICompanion.prototype.summarizeConversationsInMemory = async function() {
    ...
    try {
        ...
        const userConversations = graph.entities.filter(entity =>
            entity.name.includes(this.userId) && entity.entityType === '대화');

        // 최근 대화 10개만 요약
        const recentConversations = userConversations
            .sort((a, b) => {
                const timeA = a.observations.find(obs => obs.includes('타임스탬프'));
                const timeB = b.observations.find(obs => obs.includes('타임스탬프'));
        ...
        recentConversations.forEach(conv => {
            const topics = conv.observations.filter(obs => obs.startsWith('주제:'));
            topics.forEach(topic => {
                const topicName = topic.replace('주제: ', '');
                topicCount[topicName] = (topicCount[topicName] || 0) + 1;
            });

            const emotions = conv.observations.filter(obs => obs.startsWith('감정:'));
        ...
        return {
            ...
            topTopics: Object.keys(topicCount).sort((a, b) => topicCount[b] - topicCount[a]),
            topEmotions: Object.keys(emotionCount).sort((a, b) => emotionCount[b] - emotionCount[a]),
            topicCount,
            emotionCount
        };
    } catch (error) {
...
AICompanion.prototype.extractKeywords = function(text) {
    const stopWords = new Set([
        '이거', '그거', '저거', '뭐', '왜', '어떻게', '무엇',
        '오늘', '어제', '내일', '지금', '아침', '점심', '저녁',
        '그리고', '그런데', '그래서', '하지만', '또한',
        '我', '私', ' Pfl', ' Pfl', ' Pfl', ' Pfl', ' Pfl ', ' Pfl',
...
AICompanion.prototype.handleMemoryCommand = async function(userMessage) {
    ...
    // "대화 기록을 memory에 저장해줘"
    if (message.includes('memory') && (message.includes('저장') || message.includes('기록'))) {
        if (this.messages.length === 0) {
            this.addMessage('저장할 대화 기록이 없습니다. 😊', 'ai');
            return true;
        }

        this.addMessage('Memory MCP 서버에 대화 기록을 저장하고 있습니다... 💾', 'ai');
        ...
        this.addMessage(`✅ 총 ${savedCount}개의 대화 기록이 Memory MCP 서버에 저장되었습니다! 🧠\n\n이제 이전 대화 내용을 바탕으로 더 정확한 조언을 제공할 수 있어요.`, 'ai');
        return true;
    }

    // "이전 대화 검색해줘"
    if (message.includes('검색') && (message.includes('이전') || message.includes('과거'))) {
        this.addMessage('Memory에서 이전 대화 내용을 검색하고 있습니다... 🔍', 'ai');
        ...
        if (keywords.length > 0) {
            ...
            if (results.length > 0) {
                ...
                this.addMessage(`❌ '${keywords}' 관련 대화를 찾을 수 없습니다. 다른 키워드로 검색해보세요. 🤔`, 'ai');
            }
        } else {
            ...
            if (results.length > 0) {
                ...
                this.addMessage('저장된 대화 기록이 없습니다. 먼저 대화를 저장해주세요. 😊', 'ai');
            }
        }
        return true;
    }
    ...
    // "최근 대화 요약해줘"
    if (message.includes('요약') && (message.includes('최근') || message.includes('대화'))) {
        ...
        if (summary) {
            ...
            this.addMessage('대화 분석을 위한 데이터가 부족합니다. 더 많은 대화를 나눠보세요! 😊', 'ai');
        }
        return true;
    }
    ...
    if (message.includes('맞춤형') && message.includes('조언')) {
        this.addMessage('이전 대화 내용을 바탕으로 맞춤형 조언을 준비하고 있습니다... 🎯', 'ai');

        const advice = await this.getPersonalizedAdviceFromMemory();
        if (advice) {
            this.addMessage(advice, 'ai');
        } else {
            this.addMessage('맞춤형 조언을 제공하기 위한 데이터가 부족합니다. 더 많은 대화를 나눠보세요! 😊', 'ai');
        }
        return true;
    }
    ...
    if (message.includes('전체 기록 보기')) {
        this.addMessage('Memory MCP 서버의 전체 기록을 조회하고 있습니다... 📜', 'ai');
        try {
            const graph = await this.memoryClient.readGraph();
            if (graph && graph.entities && graph.entities.length > 0) {
                let response = '📜 **Memory MCP 서버 전체 기록:**\n\n';
                const conversationEntities = graph.entities.filter(e => e.entityType === '대화');

                if (conversationEntities.length > 0) {
                     conversationEntities.forEach((entity, index) => {
                        const contentObs = entity.observations.find(obs => obs.startsWith('내용:'));
                        const senderObs = entity.observations.find(obs => obs.startsWith('송신자:'));
                        const timeObs = entity.observations.find(obs => obs.startsWith('타임스탬프:'));

                        if (contentObs) {
                            const sender = senderObs ? senderObs.replace('송신자: ', '') : '알 수 없음';
                            const time = timeObs ? new Date(timeObs.replace('타임스탬프: ', '')).toLocaleString() : '시간 정보 없음';
                ...
                this.addMessage('저장된 기록이 없습니다. 텅 비어있어요. 🤔', 'ai');
            }
        } catch (error) {
...
AICompanion.prototype.setupKeyboardShortcuts = function() {
    ...
    if (this.settings.personality === 'counselor') {
        setTimeout(() => {
            this.addMessage('**단축키 안내:**\n• Ctrl+Shift+C: 상담가 모드로 전환\n• Ctrl+Shift+S: 대화 패턴 분석\n• Ctrl+Shift+E: 감정 일기\n• Ctrl+Shift+R: 이완 기법\n\n이 단축키들을 활용하여 더 편리하게 상담을 이용해보세요! 🎯', 'ai');
        }, 1000);
    }
};
