...
class ZAIAPIClient {
  ...
  updateAverageResponseTime(responseTime) {
    const total =
      ...
      (this.usageStats.successfulRequests + this.usageStats.failedRequests - 1);
    this.usageStats.averageResponseTime =
      (total + responseTime) /
      (this.usageStats.successfulRequests + this.usageStats.failedRequests);
  }
    ...
    if (this.memoryMCPAvailable) {
      try {
        ...
        if (keywords.length > 0) {
          const relatedConversations = await this.searchConversationsInMemory(
            keywords.join(" "),
          );
        ...
        try {
          ...
          if (chunk && chunk.choices && chunk.choices[0]) {
            ...
            if (choice.finish_reason === "stop") {
              console.log("스트림 정상 종료 [finish_reason=stop]:", {
                receivedChunks,
                totalLength: fullContent.length,
              });
              streamCompleted = true;
    ...
    if (contentElement && content) {
      try {
        ...
        contentElement.innerHTML =
          processedContent + '<span class="streaming-cursor">▊</span>';
        this.scrollToBottom();
      } catch (updateError) {
...
AICompanion.prototype.handleMemoryCommand = async function (userMessage) {
  ...
  if (
    ...
    // 키워드 추출 (주제어 제외)
    const keywords = message.replace(/검색|이전|과거|대화|내용/gi, "").trim();
          ...
          conversationEntities.forEach((entity, index) => {
            ...
            if (contentObs) {
              const sender = senderObs
                ...
                : "알 수 없음";
              const time = timeObs
                ? new Date(timeObs.replace("타임스탬프: ", "")).toLocaleString()
                : "시간 정보 없음";
